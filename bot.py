...
def get_prefix(bot, message):
    ...
    if bot.user.id == 440790222347829258:
        prefixes = ['??']
    else:
        prefixes = ['?']
...
game = discord.Game("Komennot: ?help")
dev_game = discord.Game("Development mode, commands start with ??")
...
async def on_ready():
    ...
    bot.automation_channel = bot.get_channel(606936928129908736)
    bot.moderation_log = bot.get_channel(566639351413473280)
    shard_count = bot.shard_count
    # check_folders()
    # check_files()
    #await bot.change_presence(status=discord.Status.online, activity=game)
    await bot.change_presence(activity=dev_game)
